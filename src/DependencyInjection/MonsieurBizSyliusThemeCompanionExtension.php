...
final class MonsieurBizSyliusThemeCompanionExtension extends Extension implements PrependExtensionInterface
{
    ...
    public function prepend(ContainerBuilder $container): void
    {
        ...
        $allThemes = $this->setDefault($container, [...$localThemes, ...$packagedThemes], $projectDir);
        $allThemes = $this->manageChildrenParentsMergedConfig($allThemes);
        $this->addParametersAndResources($container, $allThemes);

        $this->addAssetMapperPaths($container, $allThemes, $projectDir);
    }

    /**
     * @todo this method should be replace by something managed by Symfony\Component\Config\Definition\Processor
     */
    protected function setDefault(ContainerBuilder $container, array $themes, string $projectDir): array
    {
    ...
    protected function addAssetMapperPaths(ContainerBuilder $container, array $themes, string $projectDir): void
    {
        $excludePatterns = [];
        $assetMapperPaths = [];
